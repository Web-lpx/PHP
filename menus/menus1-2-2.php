...
$sql="select * from baozhuangdepart";
$bb=mysqli_query($conn,$sql);
$cc=mysqli_num_rows($bb);
// 读取JSON文件内容
$jsonData = file_get_contents('../layuimini/api/menus1-2-2.json');
// 解析JSON数据为PHP数组或对象
...
while($dd=mysqli_fetch_array($bb)){
	...
	$data["data"][$x]["name"]=$dd["baozhuangname"];
...
file_put_contents('../layuimini/api/menus1-2-2.json', $editedJsonData);
